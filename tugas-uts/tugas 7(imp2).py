...
def forwardPass(inputs, weight, bias, activation ='linear'):
   ...
   if activation is 'relu':
       #relu activationf(x)=max(0,x)
       act = np.maximun(w_sum,0)
       
       else :
           #linear  activation f(x)=x
           act = w_sum
           return act
#pre trained  weight & biases after trinanig
